...
public class GetActiveTransportsForShipperCompanyQueryHandler : IQueryHandler<GetActiveTransportsForShipperCompanyQuery, List<OfferInfoDto>>
{
    private readonly IUnitOfWork unitOfWork;

    public GetActiveTransportsForShipperCompanyQueryHandler(IUnitOfWork unitOfWork)
    {
        this.unitOfWork = unitOfWork;
    }

    public async Task<List<OfferInfoDto>> Handle(GetActiveTransportsForShipperCompanyQuery request, CancellationToken cancellationToken)
    {
        List<int> transportRequestIds = await unitOfWork.TransportRequests.AllNoTracking()
            .Where(t => t.CompanyId == request.CompanyId && t.TransportStatus == TransportStatus.Accepted)
            .Select(t => t.Id)
            .ToListAsync();
