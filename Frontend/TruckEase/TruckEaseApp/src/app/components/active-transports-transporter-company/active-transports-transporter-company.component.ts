...
@Component({
  ...
  templateUrl: './active-transports-transporter-company.component.html',
  styleUrls: ['./active-transports-transporter-company.component.scss']
})
export class TransporterCompanyActiveTransportsComponent implements OnInit {
  ...
  fetchTransports(): void {
    this.currentUserService.getCurrentUser()
      .pipe(
        switchMap(user => {
          return this.transportOfferApiService.getAllActiveTransportsForTransporterCompany(user.inCompanyFK);
        })
      )
      .subscribe({
        next: (result) => {
  ...
  public markAsFinished(offerId: number) : void {
    this.isLoading = true;
    this.transportOfferApiService.finishTransport(offerId)
    .subscribe({
      next: () => {
        this.isLoading = false;
        this.snackBar.open(
          'You successfully marked this transport as finished.',
          'Close',
          { duration: 5000 }
        );
        this.fetchTransports();
      ...
      error: () => {
        this.dialog.open(ErrorDialogComponent, {
          data: { message: 'Failed to mark this transport as finished. Please try again later.' },
        });
      }
    });
  }
